...
class Nimious(Plugin):
    ...
    async def nimious(self, msg, arguments):
        ww_link = self.core.emoji.emoji_str(['ww_link_ugh'])
        bow = self.core.emoji.emoji_str(['bow'])
        bomb = self.core.emoji.emoji_str(['bomb'])
        danklink = self.core.emoji.emoji_str(['dank_link'])
