...
class Nimious(Plugin):
    ...
    @command(
        "^[Ss]tryker *(?:when he)? *\"(.*)\"$", access=-1, name='Stryker'
    )
    async def stryker(self, msg, arguments):
        await self.send_message(msg.channel, "http://i.imgur.com/XpXoEDT.jpg")
        await self.send_message(msg.channel, "^ Stryker when he {}".format(msg.arguments[0]))
        await self.delete_message(msg)

    @command(
        "^(?:[Nn]imious|<@!?163167281042423810>)$", access=100, name='Nimious'
    )
